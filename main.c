...
int main(void)
{
  ...
  while (1)
  {
	  if (flag){
		 ...
		 if(val>1790&&val<1810){  //down (ms)
			 valflag = 1;
		 }
		 if(val>730&&val<760){  //up (km/h)
			 valflag = 2;
		 }
		 if(val>2840&&val<2860){  //left (mph)
			 valflag = 3;
		 }

		 //right button changes the mode, others change unit
		 if(modeflag== 1 && lcdflag==0 && valflag == 1){//m/s adc
		 	lcd16x2_priv(veloms,FFT_val.fdominant,"m/s");
		 	velo7=veloms;

		 //deciding which mode to print on lcd
		} else if(modeflag== 1 && lcdflag==0 && valflag == 2){//km/h adc
			float velokmh=veloms*3.6;
			lcd16x2_priv(velokmh,FFT_val.fdominant,"km/h");
			velo7=velokmh;

		} else if(modeflag== 1 && lcdflag==0 && valflag == 3){//mph adc
			float velomph=veloms*2.237;
			lcd16x2_priv(velomph,FFT_val.fdominant,"mph");
			velo7=velomph;
		}

		 if(modeflag== 0 && lcdflag==0 && valflag == 1){//m/s comp
			lcd16x2_priv(compvel,compfreq,"M/S");
		 	velo7=compvel;

		 //deciding which mode to print on lcd
		} else if(modeflag== 0 && lcdflag==0 && valflag == 2){//km/h comp
			float compvelkmh=veloms*3.6;
			lcd16x2_priv(compvelkmh,compfreq,"KM/H");
			velo7=compvelkmh;

		} else if(modeflag== 0 && lcdflag==0 && valflag == 3){//mph comp
		 ...
		 int x = (int)velo7;
			 int z=x/10;
			 int y=x+(z)*6;
			 tx_buffer[0] = (uint8_t)(y);
			  HAL_UART_Transmit(&huart4,tx_buffer,1,10);
